...
public partial class MainWindow
{
    ...
    [
        ...
        new("PreviewMonitor", "Preview", "Shows the photo, the exact video frame, or video playback. Roll the mouse wheel over the picture to zoom in on the spot under the pointer, drag to move around, double-click to fit again; the zoom stays as you step through frames. Ctrl with the wheel steps frames here. Click it so the Left and Right keys step frames. With Crop on, drag here to choose pixels. Messages such as Saved or an error appear at the bottom right of this area.", Preview),
        ...
        new("Timeline", "Frame timeline", "Drag the white playhead to any frame. Roll the mouse wheel over it to step through frames. Drag the amber handles to set the in and out points; the shaded part is your selection. The ruler underneath counts frames. Left and Right step one frame, Shift steps ten.", Preview),
        ...
        new("SnipAudioButton", "Snip sound to WAV", "Saves just the marked part of the sound as a WAV file in your export folder; the original file is never changed. Track and channel are chosen in the Export tab. Shown when the selected file has sound.", Preview),
        ...
        new("NextFrameButton", "Next frame", "Steps forward exactly one frame. Turning the mouse wheel over the timeline steps frames too, ten at a time with Shift; over the picture the wheel zooms, and Ctrl with the wheel steps frames. Shortcuts: period, or Right while the preview has focus.", Preview),
        ...
        new("FramingButton", "Crop & rotate video", "For changing the entire video, not just one frame. Opens tools above the timeline and puts a grabber on each of the four edges of the picture. Shown when a video is selected.", Preview),
        new("FramingTools", "Video crop and rotation tools", "Auto-fit edges finds black bars and puts the edges on the picture for you. Drag a grabber to move an edge, or click a grabber and use the arrow keys: 1 pixel per press, 10 with Shift; Tab goes to the next edge. Turn left and Turn right rotate in quarter turns and the preview turns with them. The line underneath says how much is cut and the size the video will export at. Export whole video saves a new MP4; Trim selection to MP4 uses the same crop. Your original file is never changed. These tools appear after you click Crop & rotate video.", Preview),
        new("FocusViewButton", "Focus view", "Puts away everything except the picture, the timeline and these buttons, so you can go through a video closely: zoom with the wheel over the picture, step frames with the wheel over the timeline or with comma and period, and export frames with E. Press F11 or Esc, or click Exit focus view in the button row, to get everything back as it was.", Preview),
        ...
        new("MetadataTagModeButton", "Tag from details", "Shows a checkbox beside each detail. Tick the ones you want, then confirm, to turn them into tags such as aspect ratio 16:9. Nothing is tagged automatically.", Inspector(0)),
