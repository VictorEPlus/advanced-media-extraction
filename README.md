...
- **Hover preview:** moving the pointer along the timeline, or dragging the playhead or a marker, shows a small picture of the video at that place. The pictures are made in the background once per video (a few seconds) and cached. On long videos they come from key frames, so the caption says "near frame N, showing frame M" when the picture is a neighbour; the frame you land on after clicking is always exact.
- **Playback:** Space plays and pauses. Pausing returns to an exact still of that moment; the paused video picture stays up until that still is ready, and playing again resumes the open file rather than reopening it.
...
- `cache`: PNG thumbnails, exact frame previews, per-file frame indexes (`*.index.json`), waveforms (`*.wave.bin`) and timeline preview pictures (`*.strip.bin`); disk budget defaults to 512 MB. A cache miss decodes a window of about twenty neighbouring frames in one pass so stepping stays warm; a single oversized window may exceed the budget briefly.
...
- `docs/ARCHITECTURE.md`: how frame accuracy, caching, playback hand-over, waveforms, preview pictures, zoom and the video crop work, and why.
