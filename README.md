...
- **Centre:** three folder-style tabs. **Library** shows where the media lives; **Preview** shows the selected file; **Stitch** combines a few pictures into one. The open tab is taller, has a glowing cyan top edge and joins the page below it. The selected file's name, details line, **Favorite**, **Stage** and **Focus view** sit on the same row as the tabs.
- **Inspector (right):** Details, Tags, Export and Settings. Drag its divider to resize.
- **Filmstrip (bottom):** thumbnails of the files that pass the filters, each with a small corner badge saying whether it is a photo, a video or sound. The status line under it holds **Sort**, **Thumbnail size** and **Preview follows scroll**.
...
2. The **Library** tab shows a **folder tree** of every subfolder that contains media, with counts that include subfolders, beside a **graph** of the selected folder's subfolders split into photos, videos and audio. Every folder shows its split as percentages (for example "65 photos (93%), 5 videos (7%)"), its share of its parent folder, and an album cover: the first and last picture in it. Right-click a folder for **Use its subfolders as the top level**, which flattens a folder you opened one level too high, and **Take this folder out of the library**, which hides a branch you did not want. Both only change what you are looking at; **Show all folders** puts everything back and nothing on disk is touched. Click a folder or a bar to show only that folder's files in the filmstrip; the file count names the folder and **Clear filters** returns to everything.
3. Search by file name or subfolder, filter by media type and choose a sort order. **Sort** is in the filter bar and again under the filmstrip, where the files are: name, date modified, size, media type, favorites first or full path. Natural name sorting puts `frame2` before `frame10`. A file you are inspecting stays open when a filter hides it; a **Hidden by filters** note appears.
...
| Right-click | A filmstrip thumbnail: add it to the stitch. A folder row: flatten it to its subfolders, or take it out of the library |
